...
async def handle_quiz_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handles incoming poll answers to track scores. Always resolves correct_option safely."""
    try:
        ...
        logger.info(f"[quiz] Received poll answer from {user.id} for poll {poll_id}. Selected: {selected_ids}")

        # ============================================================
        # 1️⃣ PRIMARY: Try in-memory active_polls
        # ============================================================
        poll_data = active_polls.get(poll_id)

        # ============================================================
        # 2️⃣ SECONDARY: Load from Firestore active_poll doc
        # ============================================================
        if not poll_data:
            poll_data = await _load_active_poll_from_db(poll_id)
            if poll_data:
                active_polls[poll_id] = poll_data
                logger.info(f"[quiz] Restored poll {poll_id} from Firestore into memory.")

        # ============================================================
        # 3️⃣ THIRD: Search all live_quiz_sessions for poll_id
        # ============================================================
        if not poll_data:
            if db:
                try:
                    docs = await asyncio.to_thread(db.collection("live_quiz_sessions").stream)
                    for doc in docs:
                        d = doc.to_dict() or {}
                        poll_ids = d.get("poll_ids", [])
                        if poll_id in poll_ids:
                            group_id = int(doc.id)
                            session = await _load_quiz_session_from_db(group_id) or {'scores': {}, 'poll_ids': [], 'correct_options': {}}
                            quiz_sessions[group_id] = session
                            poll_data = await _load_active_poll_from_db(poll_id)

                            if not poll_data:
                                # create placeholder so handler never crashes
                                poll_data = {
                                    "chat_id": group_id,
                                    "correct_option": None,
                                    "start_time": datetime.now(timezone.utc)
                            ...
                            active_polls[poll_id] = poll_data
                            logger.info(f"[quiz] Found poll {poll_id} inside quiz_session of chat {group_id}.")
                            break

                    if not poll_data:
                        logger.info(f"[quiz] Poll {poll_id} not found in any session → but WILL NOT ignore.")
                        # create safe fallback (we will recover correct_option later)
                        poll_data = {
                    ...
                    logger.exception(f"[quiz] Error scanning sessions for poll {poll_id}: {e}")
                    return
            else:
                logger.info(f"[quiz] No DB available → cannot restore poll {poll_id}")
                return

        # ============================================================
        # 4️⃣ Correct option resolution (UNBREAKABLE)
        # ============================================================
        chat_id = poll_data.get("chat_id", getattr(update.effective_chat, "id", 0))
        correct_id = poll_data.get("correct_option")

        # ensure session available
        session = quiz_sessions.get(chat_id)
        if not session:
            session = await _load_quiz_session_from_db(chat_id) or {}
            session.setdefault('scores', {})
            session.setdefault('poll_ids', [])
            session.setdefault('correct_options', {})
            quiz_sessions[chat_id] = session

        # 🔥 fallback A: in-memory correct_options
        if correct_id is None:
            correct_id = session.get("correct_options", {}).get(poll_id)

        # 🔥 fallback B: Firestore correct_options
        if correct_id is None and db:
            try:
                sess_doc = await asyncio.to_thread(db.collection("live_quiz_sessions").document(str(chat_id)).get)
                if sess_doc.exists:
                    co = sess_doc.to_dict().get("correct_options", {})
                    correct_id = co.get(poll_id)
                    # sync to memory
                    session.setdefault("correct_options", {}).update(co)
            except Exception as e:
                logger.warning(f"[quiz] Could not load correct_options fallback for chat {chat_id}: {e}")

        # 🔥 fallback C: Poll object (Telegram always sends correct option)
        if correct_id is None:
            try:
                telegram_poll = update.poll_answer.poll
                if telegram_poll:
                    correct_id = telegram_poll.correct_option_id
                    logger.info(f"[quiz] Recovered correct_id from Telegram poll object: {correct_id}")
            except:
                pass

        # If STILL missing → absolute last resort
        if correct_id is None:
            logger.warning(f"[quiz] correct_option STILL None for poll {poll_id}, defaulting to -1.")
            correct_id = -1

        # ============================================================
        # 5️⃣ Time calculation / scoring
        # ============================================================
        ...
        user_stats["name"] = getattr(user, 'full_name', getattr(user, 'username', user_stats["name"]))

        if correct_id in selected_ids:
            user_stats["score"] += 1
            user_stats["total_time"] += time_taken
            logger.info(f"[quiz] User {user.id} answered CORRECTLY.")
        else:
            logger.info(f"[quiz] User {user.id} answered INCORRECTLY.")

        scores[user.id] = user_stats

        # ============================================================
        # 6️⃣ Save updated session to Firestore
        # ============================================================
        try:
            await _save_quiz_session_to_db(chat_id, session)
        except Exception as e:
            logger.warning(f"[quiz] Failed to save updated scores for chat {chat_id}: {e}")
...
async def start_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    quiz_sessions[group_id] = {
        'scores': {},
        'poll_ids': [],
        # keep correct_options map in memory so fallbacks can read it quickly
        'correct_options': {}
    }

    # persist initial empty session (explicitly include correct_options to DB)
    try:
        if db:
            ref = db.collection("live_quiz_sessions").document(str(group_id))
            # Use merge to avoid overwriting other fields unintentionally
            await asyncio.to_thread(ref.set, {
                "poll_ids": quiz_sessions[group_id]['poll_ids'],
                "scores": quiz_sessions[group_id]['scores'],
                "correct_options": quiz_sessions[group_id]['correct_options'],
                "last_updated": firestore.SERVER_TIMESTAMP
            }, merge=True)
    except Exception as e:
        logger.warning(f"Could not persist initial quiz session for {group_id}: {e}")
    ...
    for i, q in enumerate(questions):
        try:
            ...
            quiz_sessions[group_id].setdefault('poll_ids', []).append(poll_key)
            quiz_sessions[group_id].setdefault('correct_options', {})[poll_key] = correct_id

            # persist both the active poll and updated session right away
            try:
                await _save_active_poll_to_db(poll_key, poll_record)
            except Exception as e:
                logger.warning(f"Failed to save active_poll {poll_key} to DB: {e}")

            # persist session with correct_options included (merge)
            try:
                if db:
                    session_ref = db.collection("live_quiz_sessions").document(str(group_id))
                    # Build a minimal session payload - merge=True ensures we don't overwrite unrelated fields
                    payload = {
                        "poll_ids": quiz_sessions[group_id]['poll_ids'],
                        "scores": quiz_sessions[group_id].get('scores', {}),
                        "correct_options": quiz_sessions[group_id].get('correct_options', {}),
                        "last_updated": firestore.SERVER_TIMESTAMP
                    }
                    await asyncio.to_thread(session_ref.set, payload, merge=True)
            except Exception as e:
                logger.warning(f"Failed to persist quiz session for {group_id}: {e}")
    ...
    try:
        ...
        if updated_session and 'scores' in updated_session:
            ...
            # keep any correct_options we have in memory, but prefer DB values if present
            quiz_sessions[group_id]['scores'] = updated_session['scores']
            # attempt to load correct_options from DB if present
            if isinstance(updated_session.get('correct_options'), dict):
                quiz_sessions[group_id].setdefault('correct_options', {}).update(updated_session.get('correct_options'))
    ...
    if group_id in quiz_sessions:
        for pid in quiz_sessions[group_id].get('poll_ids', []):
            active_polls.pop(pid, None)
            try:
                await _delete_active_poll_from_db(pid)
            except Exception as e:
                logger.debug(f"Failed to delete active_poll {pid} from DB during cleanup: {e}")
        quiz_sessions.pop(group_id, None)
        try:
            await _delete_quiz_session_from_db(group_id)
        except Exception as e:
            logger.debug(f"Failed to delete quiz_session {group_id} from DB during cleanup: {e}")
